...
func CalculateSenicScore(treeMap *[][]int, x int, y int) int {
	refTreeMap := *treeMap

	minY := 0
	maxY := len(refTreeMap) - 1

	minX := 0
	maxX := len(refTreeMap[minY]) - 1

	height := refTreeMap[y][x]
...
func StarOne(input_path string) int {
	...
	for i, line := range lines {
		for _, char := range line {
			treeMap[i] = append(treeMap[i], utils.MustParseAsInt(string(char)))
		}
	}
